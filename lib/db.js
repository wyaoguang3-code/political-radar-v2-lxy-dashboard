...
(function (root) {
  ...
  async function signalsByPlatform() {
    // 走 Postgres RPC（migration 004 建的 signals_by_platform()），server-side
    // 聚合、跳過 PostgREST 預設 1000 row limit。
    // 回傳 shape：{ facebook: {total, red, yellow, green, *_pct, updated_at}, ... }
    const c = client();
    const { data, error } = await c.rpc('signals_by_platform');
    if (error) throw error;
    ...
    PLATFORM_KEYS.forEach(function (p) {
      out[p] = (data && data[p]) || {
        ...
        red_pct: 0, yellow_pct: 0, green_pct: 0, updated_at: null,
      };
    });
    return out;
  }
