...
(function (root) {
  ...
  async function recentComments(platform, limit, hoursBack) {
    ...
    let q = c
      ...
      // comment_id added so dashboard 標錯了 modal can target social_comments row
      // (matches signals_by_platform RPC's first_seen_at-based 24h window when
      // hoursBack=24 is passed — keeps card total ≡ modal count).
